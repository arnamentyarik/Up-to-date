...
def statistics():
    ...
    stats = Player.get_statistics()
    top_players = Player.query.order_by(Player.experience.desc()).limit(3).all()
...
def achievements():
    ...
    try:
        ...
        try:
            ...
            app.logger.error(f"Error creating default achievements: {e}")

        # Get all achievements
        try:
            achievements = Achievement.query.filter_by(is_active=True).order_by(
                Achievement.rarity.desc(),
                Achievement.reward_xp.desc()
            ).all()
        except Exception as e:
        ...
        if current_player:
            try:
                ...
                Achievement.check_player_achievements(current_player)
            except Exception as e:
                app.logger.error(f"Error fetching player achievements: {e}")
                player_achievements = []
        ...
        app.logger.error(f"Critical error in achievements route: {e}")
        flash('Произошла ошибка при загрузке достижений!', 'error')
        return redirect(url_for('index'))
